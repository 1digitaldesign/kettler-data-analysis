...
```
data/
├── source/          # Source data files (authoritative datasets)
│   ├── skidmore_all_firms_complete.json      # Complete firm data (38 firms)
│   ├── skidmore_firms_database.csv           # Firm database (CSV format)
│   ├── skidmore_individual_licenses.json    # Individual licenses (JSON)
│   └── skidmore_individual_licenses.csv      # Individual licenses (CSV)
│
├── scraped/         # Scraped data from external sources
...
│
├── schema.json      # Complete schema definition (JSON Schema)
...
**Status:** ✅ 100% complete (all 38 firms verified)
