#!/usr/bin/env python3
"""
Maryland License Search - Browser Automation

Uses browser automation to search Maryland DPOR database.
"""
...
LICENSE_DIR = PROJECT_ROOT / 'research' / 'license_searches' / 'data' / 'maryland'
...
MARYLAND_URL = 'https://www.dllr.state.md.us/license/occprof/realestate.html'

def check_existing(employee: dict) -> bool:
    """Check if search already exists."""
    finding_file = LICENSE_DIR / f"md_{employee['first'].lower()}_{employee['last'].lower()}_finding.json"
    ...
    return {
        ...
        'findings': {
            f"{employee['first'].lower()}_{employee['last'].lower()}": {
                ...
                'note': 'License found' if license_found else 'No license found in Maryland'
            }
        },
        'conclusion': f"{employee['first']} {employee['last']} {'HAS' if license_found else 'DOES NOT HAVE'} a real estate license in Maryland."
    }
...
def main():
    """Main function."""
    print("Maryland License Search - Browser Automation")
    print("=" * 60)
